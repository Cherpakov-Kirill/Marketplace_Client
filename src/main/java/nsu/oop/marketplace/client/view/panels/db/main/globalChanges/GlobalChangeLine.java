...
public class GlobalChangeLine extends WindowPanel {
    ...
    public GlobalChangeLine(GlobalChangeLineListener listener, int width, int height, int posX, int posY, MarketplaceProto.DBFullChanges change) {
        ...
        this.acceptButton = ViewUtils.initButton(getPart(width,0.095), getPart(height,0.8), getPart(width,0.905), getPart(height,0.1), e -> listener.acceptTheChange(change.getId()));
        add(acceptButton);
    ...
    public void markAsAccepted() {
        remove(acceptButton);
        setImageIcon("/client/acceptGlobalChanges/AcceptedGlobalChangesTable.png");
    }
}
